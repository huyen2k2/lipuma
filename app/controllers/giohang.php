<?php 
	...
	class giohang extends DController{
		...
		public function dathang(){
			...
			if(Session::get("shopping_cart")==true){
				foreach(Session::get("shopping_cart") as $key => $value){
					...
					$this->update_product_quantity($value['product_id'], $value['product_quantity'], $categorymodel);
		...
		public function update_product_quantity($id, $product_ordered_quantity, $categorymodel){
			$products_ordered = $categorymodel->getproductbyid('tbl_product', $id);

			foreach($products_ordered as $key => $product) {
				$table = "tbl_product";
				$cond = "id_product='$id'";
				$quantity = (int)$product['quantity_product'] - (int)$product_ordered_quantity;
